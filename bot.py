...
def block(name,phase,done,total,speed):
...
async def settings(_,m):
    ...
    btn = [[
        InlineKeyboardButton("📄 Upload as Document" + (" ✅" if mode=="doc" else ""), callback_data="mode_doc"),
        InlineKeyboardButton("🎥 Upload as Video"    + (" ✅" if mode=="vid" else ""), callback_data="mode_vid")
    ]]
    await m.reply_text("⚙️ Select upload preference:", reply_markup=InlineKeyboardMarkup(btn))
...
async def cb_mode(_,q):
    ...
    await q.answer("✅ Preference updated")
...
async def file_lookup(_,m):
    ...
    if not res: return await m.reply_text("❌ No matches found")
    await m.reply_text(f"📂 Found {len(res)} result(s); sending …")
    for f in res:
...
# ========= MESSAGE / LINK DETECTOR (also groups) =========
@bot.on_message(filters.regex(r"https?://\S+") & ~filters.edited)
...
async def process_link(m,url):
    ...
    try:
        # === handle m3u8 ===
        if ".m3u8" in url:
            filename="video.mp4"; path=os.path.join(tmp,filename)
                ...
                async with s.get(url,allow_redirects=True) as r:
                    ...
                    with open(path,"wb") as f:
                        async for chunk in r.content.iter_chunked(1024*512):
                            ...
                            now=time.time()
                            if now-last>10:  # every 10 s
                                spd=done/max(now-start,1)
                                try: await msg.edit_text(block(filename,"⬇️ Downloading",done,total,spd))
        ...
        if not logm: return await msg.edit_text("⚠️ Failed to save in Logs")
...
if __name__ == "__main__":
    print("💠 SERENA booting — Render health‑port + Telegram polling together")
