...
async def start(_, m):
    ...
    welcome = (
        ...
        "✨ Paste any direct download link or even an `.m3u8` stream URL — I'll grab it for you.\n"
        "🎞 All files are sent here and a backup goes to logs.\n\n"
        "🧭 Use `/help` to learn commands and don’t forget to join the update channel below 💖"
    )
...
async def help_cmd(_, m):
    txt = (
        ...
        "📦 Once done, I send you the file and save it in my Logs channel.\n\n"
        "⚙️ Commands:\n"
        "`/start` – welcome menu\n"
        "`/help` – this guide\n"
        "`/settings` – choose upload mode (video/document)\n"
...
async def settings(_, m):
    ...
    desc = (
        "⚙️ **SERENA Settings**\n\n"
        "Choose how I send files to you:\n"
        "🎥 *Upload as Video* – everything playable.\n"
        "📄 *Upload as Document* – keeps original form.\n\n"
...
async def file_cmd(_, m):
    ...
    if not fs:
        return await m.reply_text("❌ No matches found.")
